...
async function main() {
    ...
    const DAIAmountOut = ethers.parseUnits("2000", 6);

    const approveTx =
        await USDC.connect(impersonatedSigner).approve(UniSwapRouter, DAIAmountOut);
    ...
    const swapTx =
        await ROUTER.connect(impersonatedSigner).swapExactTokensForTokens(
            DAIAmountOut,
            0
            ,
